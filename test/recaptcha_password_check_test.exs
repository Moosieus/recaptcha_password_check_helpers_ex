defmodule RecaptchaPasswordCheckTest do
    ...
    def respond(verification, leaked_credentials) do
      ...
      prefixes =
        Enum.map(leaked_credentials, fn {username, password} ->
          username
          |> RecaptchaPasswordCheck.canonicalize_username()
  ...
  defp create!(username, password) do
    {:ok, verification} = RecaptchaPasswordCheck.create(username, password)
  ...
  describe "create/2" do
    test "returns a populated struct" do
      assert {:ok, verification} = RecaptchaPasswordCheck.create("foo", "bar")

      assert %RecaptchaPasswordCheck{} = verification
    ...
    test "reports a username that canonicalizes to nothing" do
      assert RecaptchaPasswordCheck.create("@nolocalpart", "pw") ==
               {:error, :empty_canonical_username}
    end

    test "generates a distinct key per verification" do
      refute create!("foo", "bar").private_key == create!("foo", "bar").private_key
    end

    test "rejects credentials that are empty or not binaries" do
      for args <- [{"", "bar"}, {"foo", ""}, {nil, "bar"}, {"foo", nil}] do
        {username, password} = args

        assert_raise FunctionClauseError, fn ->
          RecaptchaPasswordCheck.create(username, password)
        end
      end
    end
  ...
  describe "leaked?/3" do
    test "reports a leak when the bucket contains the credentials" do
      ...
      assert RecaptchaPasswordCheck.leaked?(verification, reencrypted, prefixes)
    end
    ...
    test "reports no leak when the password differs" do
      ...
      refute RecaptchaPasswordCheck.leaked?(verification, reencrypted, prefixes)
    end
    ...
    test "an empty bucket answers without touching the reencrypted hash" do
      ...
      refute RecaptchaPasswordCheck.leaked?(verification, "not a valid curve point", [])
    end
    ...
    test "a raw address canonicalized by the facade matches the canonical leak entry" do
      {:ok, verification} =
        RecaptchaPasswordCheck.create("Foo.Bar@example.com", "bar")
    ...
    test "another verification's key cannot read the response" do
      ...
      refute RecaptchaPasswordCheck.leaked?(other, reencrypted, prefixes)
    end
    ...
    test "rejects an empty reencrypted hash" do
      ...
      assert_raise FunctionClauseError, fn ->
        RecaptchaPasswordCheck.leaked?(verification, "", [<<1>>])
      end
    end
  end
  ...
  describe "canonicalize_username/1" do
    test "leaves lowercase ASCII alone" do
      assert RecaptchaPasswordCheck.canonicalize_username("test") == "test"
    end
    ...
    test "strips dots" do
      assert RecaptchaPasswordCheck.canonicalize_username("test.test") == "testtest"
    end

    test "strips an email host" do
      assert RecaptchaPasswordCheck.canonicalize_username("test@example.com") == "test"
    end

    test "strips the host before stripping dots" do
      assert RecaptchaPasswordCheck.canonicalize_username("test.name@ex.com") == "testname"
    end

    test "does not case-fold non-ASCII characters" do
      assert RecaptchaPasswordCheck.canonicalize_username("äöü日本語العَرَبِيَّة") == "äöü日本語العَرَبِيَّة"
    end
    ...
    test "folds only the ASCII portion of a mixed username" do
      assert RecaptchaPasswordCheck.canonicalize_username("Änna.Smith@example.com") == "Ännasmith"
    end

    test "strips an internationalized host" do
      assert RecaptchaPasswordCheck.canonicalize_username("例え@例え.テスト") == "例え"
    end

    test "keeps only the part before the first @" do
      assert RecaptchaPasswordCheck.canonicalize_username("a@b@c") == "a"
    end

    test "yields an empty username when the local part is empty" do
      assert RecaptchaPasswordCheck.canonicalize_username("@nolocalpart") == ""
    end

    test "is idempotent" do
      for username <- ["Foo.Bar@example.com", "a@b@c", "äöü", "@nolocalpart"] do
        once = RecaptchaPasswordCheck.canonicalize_username(username)

        assert RecaptchaPasswordCheck.canonicalize_username(once) == once
      end
    end
  end
end
